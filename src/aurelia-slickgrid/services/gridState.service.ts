...
export class GridStateService {
  ...
  subscribeToAllGridChanges(grid: any) {
    ...
    this._subscriptions.push(
      this.pluginEa.subscribe('filterService:filterChanged', (currentFilters: CurrentFilter[]) => {
        ...
        this.pluginEa.publish('gridStateService:changed', { change: { newValues: currentFilters, type: GridStateType.filter }, gridState: this.getCurrentGridState({ requestRefreshRowFilteredRow: !this.hasRowSelectionEnabled() }) });
  ...
  private reEvaluateRowSelectionAfterFilterChange() {
    ...
    if (!isequal(this._selectedFilteredRowDataContextIds, previousSelectedFilteredRowDataContextIds)) {
      ...
      this.pluginEa.publish('gridStateService:changed', { change: { newValues, type: GridStateType.rowSelection }, gridState: this.getCurrentGridState({ requestRefreshRowFilteredRow: false }) });
    }
  }
