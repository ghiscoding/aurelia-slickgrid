...
describe('FilterService', () => {
  ...
  describe('callbackSearchEvent (private) method', () => {
    ...
    it('should execute the callback normally when a keyup event is triggered and searchTerms are defined', () => {
      ...
      const spySearchChange = jest.spyOn(service.onSearchChange as SlickEvent, 'notify');

      service.init(gridStub);
      service.bindLocalOnFilter(gridStub);
      gridStub.onHeaderRowCellRendered.notify(mockArgs, new Slick.EventData(), gridStub);
      service.getFiltersMetadata()[0].callback(DOM.createCustomEvent('input'), { columnDef: mockColumn, operator: 'EQ', searchTerms: ['John'], shouldTriggerQuery: true });
    ...
    it('should delete the column filters entry (from column filter object) when searchTerms is empty', () => {
      ...
      service.bindLocalOnFilter(gridStub);
      gridStub.onHeaderRowCellRendered.notify(mockArgs, new Slick.EventData(), gridStub);
      service.getFiltersMetadata()[0].callback(DOM.createCustomEvent('input'), { columnDef: mockColumn, operator: 'EQ', searchTerms: [''], shouldTriggerQuery: true });
  ...
  describe('clearFilter methods on backend grid', () => {
    ...
    beforeEach(() => {
      ...
      service.getFiltersMetadata()[1].callback(DOM.createCustomEvent('input'), { columnDef: mockColumn3 });
      service.getFiltersMetadata()[0].callback(DOM.createCustomEvent('input'), { columnDef: mockColumn1, operator: 'EQ', searchTerms: ['John'], shouldTriggerQuery: true });
      service.getFiltersMetadata()[1].callback(DOM.createCustomEvent('input'), { columnDef: mockColumn2, operator: 'NE', searchTerms: ['Doe'], shouldTriggerQuery: true });
    });
  ...
  describe('disableFilterFunctionality method', () => {
    ...
    it('should disable the Filter Functionality from the Grid Options & toggle the Header Filter row', () => {
      ...
      (gridOptionMock.gridMenu!.customItems as GridMenuItem[]).forEach(item => {
  ...
  describe('Tree Data View', () => {
    ...
    describe('bindLocalOnFilter method', () => {
      ...
      it('should return True when item is found and its parent is not collapsed', () => {
        ...
        expect(eaSpy).toHaveBeenCalledWith(`filterService:filterChanged`, [{ columnId: 'file', searchTerms: ['map',] }]);
        expect(output).toBe(true);
      ...
      it('should return False when item is not found in the dataset', () => {
        ...
        expect(eaSpy).toHaveBeenCalledWith(`filterService:filterChanged`, [{ columnId: 'file', searchTerms: ['unknown',] }]);
        expect(output).toBe(false);
        expect(preFilterSpy).toHaveBeenCalledWith(dataset, columnFilters);
