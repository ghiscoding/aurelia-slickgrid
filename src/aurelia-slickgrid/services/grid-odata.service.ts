...
export class GridOdataService implements BackendService {
  ...
  private odataDateTimeWrapper(dateValue: string, version: number) {
    return version >= 4 ? dateValue : `DateTime'${dateValue}'`;
  }
  ...
  private filterBySearchDate(fieldName: string, operator: OperatorType | OperatorString, searchTerms: SearchTerm[], version: number): string {
    ...
    if (!Array.isArray(searchValues) && Array.isArray(searchTerms) && searchTerms.length === 1 && searchTerms[0]) {
      const searchValue1 = this.odataDateTimeWrapper(parseUtcDate(searchTerms[0] as string, true), version);
      if (searchValue1) {
    ...
    if (Array.isArray(searchValues) && searchValues.length === 2 && searchValues[0] && searchValues[1]) {
      ...
      const searchValue1 = this.odataDateTimeWrapper(parseUtcDate(searchValues[0] as string, true), version);
      const searchValue2 = this.odataDateTimeWrapper(parseUtcDate(searchValues[1] as string, true), version);
