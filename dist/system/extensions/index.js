System.register(["./autoTooltipExtension", "./cellExternalCopyManagerExtension", "./checkboxSelectorExtension", "./columnPickerExtension", "./extensionUtility", "./gridMenuExtension", "./groupItemMetaProviderExtension", "./headerButtonExtension", "./headerMenuExtension", "./rowMoveManagerExtension", "./rowSelectionExtension"], function (exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    function exportStar_1(m) {
        var exports = {};
        for (var n in m) {
            if (n !== "default") exports[n] = m[n];
        }
        exports_1(exports);
    }
    return {
        setters: [
            function (autoTooltipExtension_1_1) {
                exportStar_1(autoTooltipExtension_1_1);
            },
            function (cellExternalCopyManagerExtension_1_1) {
                exportStar_1(cellExternalCopyManagerExtension_1_1);
            },
            function (checkboxSelectorExtension_1_1) {
                exportStar_1(checkboxSelectorExtension_1_1);
            },
            function (columnPickerExtension_1_1) {
                exportStar_1(columnPickerExtension_1_1);
            },
            function (extensionUtility_1_1) {
                exportStar_1(extensionUtility_1_1);
            },
            function (gridMenuExtension_1_1) {
                exportStar_1(gridMenuExtension_1_1);
            },
            function (groupItemMetaProviderExtension_1_1) {
                exportStar_1(groupItemMetaProviderExtension_1_1);
            },
            function (headerButtonExtension_1_1) {
                exportStar_1(headerButtonExtension_1_1);
            },
            function (headerMenuExtension_1_1) {
                exportStar_1(headerMenuExtension_1_1);
            },
            function (rowMoveManagerExtension_1_1) {
                exportStar_1(rowMoveManagerExtension_1_1);
            },
            function (rowSelectionExtension_1_1) {
                exportStar_1(rowSelectionExtension_1_1);
            }
        ],
        execute: function () {
        }
    };
});
//# sourceMappingURL=index.js.map