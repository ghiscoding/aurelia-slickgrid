...
_slickgridEs.Slick.RowSelectionModel = RowSelectionModel;
...
function RowSelectionModel(options) {
  ...
  function rowsToRanges(rows) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ranges.push(new _slickgridEs.Slick.Range(rows[i], 0, rows[i], lastCell));
    }
    return ranges;
  }
  ...
  function handleActiveCellChange(e, data) {
    if (_options.selectActiveRow && data.row != null) {
      setSelectedRanges([new _slickgridEs.Slick.Range(data.row, 0, data.row, _grid.getColumns().length - 1)]);
    }
  }
